  ...
  try {
    ...
    dispatch_email(
      ...
      "<h3>Congratulations! " +
        email +
        "!" +
        " , </h3><p><h3>You can now now use truck extension </h3></p>",
      ""
    );
  } catch (error) {
    dispatch_email(
      ...
      "<h3>Payment Failed!" +
        email +
        ` </h3><h3>${error.raw.message}<p>Please try again after some time </p></h3>`,
      ""
    );
...
const checkReminder = () => {
  ...
  SubscriptionStatus.find().then((res) => {
    res.forEach((ele) => {
      ...
      const formatCurrentDate1 = currentDate1.toLocaleDateString("en-US", {
        year: "numeric",
        month: "numeric",
        day: "numeric",
      });
      const formatCurrentDate2 = currentDate2.toLocaleDateString("en-US", {
        year: "numeric",
        month: "numeric",
        day: "numeric",
      });
        ...
        Customer.find({ id: ele.customer }).then((customer) => {
          customer.forEach((customers) => {
            ...
            dispatch_email(
              customers.email,
              "Reminder Email!",
              `<h3>Reminder Email!  ${customers.email}   </h3><h3><p>Counter Extension Subscription is going to expire on </p></h3> <p>Please Click on the button to re-subscribe and continue using Counter Extension<p><br><input type="button" value="Subscribe" style="color:#ffffff;background-color: #21AD22;border-color:#21AD22;padding: 15px;font-weight:700;border-radius: 10px;border:1px solid">`,
              formattedDate
            );
          });
        });
